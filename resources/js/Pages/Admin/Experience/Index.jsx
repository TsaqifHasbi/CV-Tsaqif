            ...
            <div className="space-y-6">
                ...
                <div className="glass-card rounded-xl overflow-hidden">
                    {experiences?.length ? (
                        <table className="data-table">
                            ...
                            <tbody>
                                {experiences.map((item) => (
                                    <tr key={item.id}>
                                        <td className="text-gray-900 font-medium">{item.job_title}</td>
                                        <td className="text-gray-600">{item.organization}</td>
                                        <td className="text-gray-600">{formatDate(item.start_date)} - {item.is_current ? 'Present' : formatDate(item.end_date)}</td>
                        ...
                        <div className="p-12 text-center"><p className="text-gray-500 mb-4">No experience entries yet</p><Link href={route('admin.experience.create')} className="btn-primary">Add Your First Experience</Link></div>
                    )}
                </div>
            </div>
        </AdminLayout>
