...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        job_title: '', organization: '', start_date: '', end_date: '', is_current: false, description: '', location: '', employment_type: '', order: 0, is_active: true,
    });
            ...
            <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
                <div className="glass-card p-6 rounded-xl space-y-6">
                    <div className="grid md:grid-cols-2 gap-6">
                        <div><label className="form-label">Job Title *</label><input type="text" value={data.job_title} onChange={(e) => setData('job_title', e.target.value)} className="form-input" required />{errors.job_title && <p className="text-red-400 text-sm mt-1">{errors.job_title}</p>}</div>
                    ...
                    <div className="flex items-center gap-2"><input type="checkbox" id="is_active" checked={data.is_active} onChange={(e) => setData('is_active', e.target.checked)} className="rounded bg-dark-700 border-dark-600" /><label htmlFor="is_active" className="text-gray-300">Active</label></div>
                </div>
